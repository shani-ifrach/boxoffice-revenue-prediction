...
- Select model families by mean rolling-validation MAE for regression and PR-AUC
  for rare-event classification. The current trainer excludes 2022–2024 from model
  selection. Earlier exploratory stability work inspected this period, so the project
  does not describe it as a never-inspected test set.
- Retain Random Forest as the production revenue model. It has the lowest mean
  rolling-validation MAE in the expanded seven-model comparison; the separate
  2022–2024 evaluation also supports, but does not determine, this choice.
- Optimize classifier thresholds for at least 80% validation recall, then precision.
...
- Use permutation-compatible model pipelines; impurity importance is not presented
  as causal evidence. SHAP is omitted because no stable SHAP output is delivered.
...
- Search a 0.005-step classification-threshold grid across the complete `[0, 1]`
  range.
